...
st.set_page_config(page_title="Monitor ARCA Senior - Auditoría Real Time", layout="wide")
...
# --- 2. SIDEBAR CON ÍNDICES CRÍTICOS ACTUALIZADOS ---
with st.sidebar:
    ...
    # Valores actualizados a la dinámica de Dic 2025
    ...
    st.metric("Tasa Desempleo", "6.6%", "Bajo")
    ...
    if st.button("🔄 Sincronizar Sistemas"):
        st.cache_data.clear()
        st.rerun()
    st.caption("Fuente: BCRA / INDEC / Bolsar")
...
# --- 4. TIPOS DE CAMBIO DINÁMICOS (Evita IndexError) ---
...
with ce:
    ...
    noticias_e = [
        ...
        ("Bonos: Licitación Tesoro exitosa", "https://www.argentina.gob.ar/noticias"),
...
# --- 9. MONOTRIBUTO (TABLA COMPLETA A-K) ---
st.subheader("⚖️ Monotributo: Escalas Vigentes 2025")
df_mono = pd.DataFrame({
    "Categoría": ["A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K"],
    "Ingresos Anuales ($)": ["8.98M", "13.34M", "18.67M", "23.21M", "27.32M", "34.11M", "40.87M", "62.01M", "69.45M", "79.44M", "94.805.682,90"],
...
# --- 10. RG 830: RETENCIONES (ENAJENACIÓN $224k) ---
st.subheader("📋 Retenciones Ganancias: RG 830")
data_rg = {
    "Concepto de Pago": ["Enajenación de Bienes Muebles", "Locaciones de Obra/Servicios", "Honorarios Profesionales", "Comisiones", "Alquileres"],
    "Mínimo No Sujeto ($)": ["224.000,00", "98.240,00", "98.240,00", "45.100,00", "16.360,00"],
    "Alícuota Insc. (%)": ["2,0%", "2,0%", "Escala Art. 94", "3,0%", "6,0%"],
    "Alícuota No Insc. (%)": ["25%", "28%", "28%", "28%", "28%"]
}
